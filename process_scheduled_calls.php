...
try {
    ...
    error_log("[" . $currentTime . "] Starting scheduled calls check");
    ...
    error_log("[" . $currentTime . "] Found " . count($dueCalls) . " due calls to process");
    
    foreach ($dueCalls as $call) {
        try {
            error_log("[" . $currentTime . "] Processing call ID: " . $call['id'] . " to " . $call['phone_number']);
            ...
            error_log("[" . $currentTime . "] Call completed successfully for ID: " . $call['id']);
            
        } catch (Exception $e) {
            error_log("[" . $currentTime . "] Error processing call ID " . $call['id'] . ": " . $e->getMessage());
    ...
    error_log("[" . date('Y-m-d H:i:s') . "] Error in process_scheduled_calls.php: " . $e->getMessage());
}
?> 